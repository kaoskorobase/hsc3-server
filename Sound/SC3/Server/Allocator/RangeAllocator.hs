...
sa_free :: (Enum i, NFData i, Num i, Ord i, Monad m) => i -> SimpleAllocator i -> m (SimpleAllocator i)
...
sa_alloc_ (SimpleAllocator r u n f) | n == upperBound r = return (n, SimpleAllocator r u (lowerBound r) f)
                                    | otherwise         = return (n, SimpleAllocator r u (succ n)       f)

sa_free_ :: (Monad m) => i -> SimpleAllocator i -> m (SimpleAllocator i)
sa_free_ _ = return

instance (Enum i, NFData i, Num i, Ord i) => IdAllocator i (SimpleAllocator i) where
    alloc = sa_alloc_
    free  = sa_free_
